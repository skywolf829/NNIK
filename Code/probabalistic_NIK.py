...
class primary_network(nn.Module):
    ...
    '''
    def __init__(self, n_inputs, n_GMM=50, nodes_per_layer=256):
        super().__init__()
    ...
    def set_weights(self, weight_array):
        ...
        spot = 0
        # first layer weights
        self.net[0].weight = weight_array[
            spot:spot+self.n_inputs*self.nodes_per_layer
            ].reshape(
                [self.n_inputs, self.nodes_per_layer]
                )
        spot += self.n_inputs*self.nodes_per_layer
        # first layer biases
        self.net[0].bias = weight_array[spot:spot+self.nodes_per_layer]
        spot += self.nodes_per_layer
        
        # second layer weights
        self.net[2].weight = weight_array[
            spot:spot+self.nodes_per_layer*self.nodes_per_layer
            ].reshape(
                [self.nodes_per_layer, self.nodes_per_layer]
                )
        spot += self.nodes_per_layer*self.nodes_per_layer
        # second layer biases
        self.net[2].bias = weight_array[spot:spot+self.nodes_per_layer]
        spot += self.nodes_per_layer
        
        # third layer weights
        self.net[4].weight = weight_array[
            spot:spot+self.nodes_per_layer*self.nodes_per_layer
            ].reshape(
                [self.nodes_per_layer, self.nodes_per_layer]
                )
        spot += self.nodes_per_layer*self.nodes_per_layer
        # third layer biases
        self.net[4].bias = weight_array[spot:spot+self.nodes_per_layer]
        spot += self.nodes_per_layer
        
        # fourth layer weights
        self.net[6].weight = weight_array[
            spot:spot+self.nodes_per_layer*(3*self.n_GMM)
            ].reshape(
                [self.nodes_per_layer, 3*self.n_GMM]
                )
        spot += self.nodes_per_layer*(3*self.n_GMM)
        # fourth layer biases
        self.net[6].bias = weight_array[spot:spot+(3*self.n_GMM)]
        spot += 3*self.n_GMM        
...
class probabalistic_NIK(nn.Module):  
    ...
    '''
    def __init__(self, n_segments=3, n_GMM=50, 
        ...
        self.main_component = nn.Sequential(
            nn.Linear(1, self.nodes_per_layer),
            nn.ReLU(),
        ...
        for i in range(self.n_segments):
            self.projection_layers.append(
                nn.Linear(self.primary_network_nodes_per_layer, # the output from the main component 
                          i*self.primary_network_nodes_per_layer + \
    ...
    def sample_gmm(self, gmm):
        ...
        sigma = gmm[range(gmm.shape[0]),sampled_gmm+1]
        sampled = torch.normal(mu, sigma)
        return sampled        

    def forward(self, x):
        '''
        Does probabalistic IK for x
        '''
        ...
        for i in range(len(self.projection_layers)):
            ...
            primary_network_input = torch.zeros([x.shape[0], max(i, 1)])
            if(i > 0):
                primary_network_input[:] = torch.tensor(sampled_configuration)
